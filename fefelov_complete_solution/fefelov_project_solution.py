...
All files and paths prefixed with fefelov for multi-user environment
"""
...
landing_to_bronze_task = SparkSubmitOperator(
    ...
    application='fefelov_src/fefelov_batch/fefelov_landing_to_bronze.py',
    ...
    env_vars={
        ...
        'DATA_PATH': './data',
        'PYTHONPATH': '/opt/spark/jobs/fefelov_src',
    },
    dag=dag,
)
...
bronze_to_silver_task = SparkSubmitOperator(
    ...
    application='fefelov_src/fefelov_batch/fefelov_bronze_to_silver.py',
    conn_id='spark-default',
    verbose=True,
    application_args=[],
    conf={
...
silver_to_gold_task = SparkSubmitOperator(
    ...
    application='fefelov_src/fefelov_batch/fefelov_silver_to_gold.py',
    conn_id='spark-default',
    verbose=True,
    application_args=[],
    conf={
...
## Landing to Bronze Task (fefelov)
...
### Input: FTP CSV files
### Output: Bronze layer Parquet files (fefelov prefixed)
"""

bronze_to_silver_task.doc_md = """
## Bronze to Silver Task (fefelov)
...
### Input: Bronze layer Parquet files (fefelov prefixed)
### Output: Silver layer Parquet files (fefelov prefixed)
"""

silver_to_gold_task.doc_md = """
## Silver to Gold Task (fefelov)
...
### Input: Silver layer Parquet files (fefelov prefixed)
### Output: Gold layer analytical dataset (fefelov prefixed)
"""
...
All files and directories are prefixed with 'fefelov_' to avoid conflicts in the shared environment.

## Architecture:
- **Landing**: Raw CSV files from FTP
- **Bronze**: Raw data in Parquet format (fefelov_athlete_bio, fefelov_athlete_event_results)
- **Silver**: Cleaned and deduplicated data (fefelov_athlete_bio_clean, fefelov_athlete_event_results_clean)
- **Gold**: Analytical datasets ready for consumption (fefelov_avg_stats)
...
All resources (tables, topics, DAGs, files, directories) are prefixed with "fefelov" 
to avoid conflicts with other students on the shared GoIT cloud infrastructure.

## File Structure:
- fefelov_src/fefelov_batch/fefelov_landing_to_bronze.py
- fefelov_src/fefelov_batch/fefelov_bronze_to_silver.py
- fefelov_src/fefelov_batch/fefelov_silver_to_gold.py
- fefelov_src/fefelov_common/fefelov_config.py
- fefelov_src/fefelov_common/fefelov_spark_manager.py
- fefelov_src/fefelov_common/fefelov_utils.py
"""
