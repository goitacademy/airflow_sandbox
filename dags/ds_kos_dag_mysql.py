...
with DAG(
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS kostiya;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS kostiya.games (
        edition text,
    ...
    # Сенсор для порівняння кількості рядків у таблицях `anatolii.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from kostiya.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `kostiya.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE kostiya.games;
            INSERT INTO kostiya.games SELECT * FROM olympic_dataset.games;
        """,
    )
