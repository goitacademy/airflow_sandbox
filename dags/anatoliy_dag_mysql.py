...
with DAG(
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS anatolii;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS anatolii.games (
        edition text,
    ...
    check_for_data = SqlSensor(
        ...
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from anatolii.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `anatolii.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE anatolii.games;
            INSERT INTO anatolii.games SELECT * FROM olympic_dataset.games;
        """,
    )
