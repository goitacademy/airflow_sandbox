...
Simplified version that works with existing Airflow environment
"""
...
landing_to_bronze_task = SparkSubmitOperator(
    ...
    application='/opt/airflow/jobs/fefelov_landing_to_bronze.py',
...
bronze_to_silver_task = SparkSubmitOperator(
    ...
    application='/opt/airflow/jobs/fefelov_bronze_to_silver.py',
    conn_id='spark-default',
    verbose=True,
    application_args=[],
    conf={
...
silver_to_gold_task = SparkSubmitOperator(
    ...
    application='/opt/airflow/jobs/fefelov_silver_to_gold.py',
    conn_id='spark-default',
    verbose=True,
    application_args=[],
    conf={
...
All resources (tables, topics, DAGs, files) are prefixed with "fefelov" 
to avoid conflicts with other students on the shared GoIT cloud infrastructure.

## Note:
This DAG expects the batch processing scripts to be available in the 
Airflow environment at /opt/airflow/jobs/ with fefelov_ prefix.
"""
