...
connection_name = "goit_mysql_db_tsygankov"

# Визначення DAG
with DAG(
        'working_with_mysql_db',
        default_args=default_args,
        schedule_python --versioninterval=None,  # DAG не має запланованого інтервалу виконання
        catchup=False,  # Вимкнути запуск пропущених задач
        tags=["tsygankov"]  # Теги для класифікації DAG
) as dag:
    ...
    create_schema = MySqlOperator(
        ...
        sql="""
        CREATE DATABASE IF NOT EXISTS tsygankov;
        """
    )
    ...
    create_table = MySqlOperator(
        ...
        mysql_conn_id=connection_name,
        sql="""
        CREATE TABLE IF NOT EXISTS tsygankov.games (
        `edition` text,
    ...
    # Сенсор для порівняння кількості рядків у таблицях `tsygankov.games` і `olympic_dataset.games`
    check_for_data = SqlSensor(
        task_id='check_if_counts_same',
        conn_id=connection_name,
        sql="""WITH count_in_copy AS (
                select COUNT(*) nrows_copy from tsygankov.games
                ),
    ...
    # Завдання для оновлення даних у таблиці `tsygankov.games`
    refresh_data = MySqlOperator(
        task_id='refresh',
        mysql_conn_id=connection_name,
        sql="""
            TRUNCATE tsygankov.games;  # Очищення таблиці
            INSERT INTO tsygankov.games SELECT * FROM olympic_dataset.games;  # Вставка даних з іншої таблиці
        """,
    )
